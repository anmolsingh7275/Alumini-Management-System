...
import { registerAlumniAPI } from "../../services/authService";

export default function AlumniRegister() {
  const [form, setForm] = useState({ name: "", email: "", password: "", batch: "", department: "", gender: "male", company: "" });
  const navigate = useNavigate();

  async function submit(e) {
    e.preventDefault();
    try {
      await registerAlumniAPI(form);
  ...
  return (
    <form onSubmit={submit} className="bg-white dark:bg-gray-800 p-6 rounded shadow">
      <h2 className="text-lg font-semibold mb-4">Alumni Registration</h2>
      ...
      <input className="w-full mb-2 p-2 border rounded" placeholder="Batch (e.g. 2019)" value={form.batch} onChange={(e)=>setForm({...form, batch:e.target.value})}/>
      ...
      <button className="w-full bg-blue-600 text-white py-2 rounded">Register as Alumni</button>
    </form>
  );
}
